...
```
                  ┌─────────────┐    ┌──────────────┐
     raw image ──►│ Qwen2.5 ViT │──► │   merger     │──► image_embeds ─┐
                  │  (frozen)   │    │  (visual     │                  │
                  └─────────────┘    │   adapter;   │                  │
                                     │   trainable) │                  │
                                     └──────────────┘                  ├─► attention-fused
                                                                       │   image_embeds → LLM
                  ┌─────────────┐    ┌──────────────┐                  │
     raw image ──►│   Expert    │──► │  expert_     │──► 1 vec (hidden)┘
                  │ (DINOv2 /   │    │  projector   │
                  │  PLIP /     │    │ (trainable)  │
                  │  Wyze;      │    └──────────────┘
                  │  frozen)    │
...
Two trainable adapters bridge the frozen towers to Qwen's hidden space:
- **merger (visual adapter)** — Qwen's `Qwen2_5_VLPatchMerger`. Groups every 2×2 block of post-ViT patch tokens into one output token (4× sequence reduction) and projects to the LLM hidden dim.
- **expert_projector** — a small MLP that lifts the expert's pooled vector into the same hidden dim as `image_embeds`, so the two streams can be mixed token-for-token.
...
- `merger` — Qwen's vision-language merger / visual adapter (default, essential)
...
- `post_fusion_adapter` — the zero-init residual MLP applied to fused image_embeds (`input_mode='expert_and_image_attn_adapter'`)
...
### 5. `expert_and_image_attn_adapter` + `expert` — attn fusion + post-fusion residual adapter

Same convex-combination fusion as `expert_and_image_attn`, with an extra **zero-init residual MLP adapter** applied to the fused `image_embeds` before they reach the LLM. The adapter is initialised so that at step 0 it is the identity (no perturbation vs. the `expert_and_image_attn` baseline) and only grows off zero if training finds the post-fusion distribution gap worth closing.

The mode now has **three trainable adapters** between the frozen towers and the LLM input:
...
Math, per image, in [replace.py](replace.py):
```
image_embeds = (1 − α) * image_embeds + α * broadcasted_expert_feature   # same as mode 2
image_embeds = post_fusion_adapter(image_embeds)                          # x + fc2(GELU(fc1(LN(x)))), fc2 zero-init
```

The adapter is a bottleneck MLP (`hidden → hidden/4 → hidden`) wrapped in a residual connection, with the last linear layer's weights and bias initialised to zero — see `PostFusionAdapter` in [utils/model_utils.py](utils/model_utils.py).

Recommended starting setup:

```
--input_mode expert_and_image_attn_adapter --feature_mode expert
--training_parameters merger expert_projector post_fusion_adapter
--warmup_connector_path <path_to_prior_attn_run>/connector.pt   # loads only 'expert_projector'
```

Trainable-parameter count is small (≈ `2 * hidden_dim^2 / 4` ≈ a few M params for a 7B model), so the new mode adds little compute or memory vs. plain `expert_and_image_attn`.

All previously listed input modes (`image_only`, `expert_and_image_attn`, `expert_and_image_add`, `expert_only`, `expert_and_image_concat`, `expert_cross_attn`, `expert_qformer`) are untouched; the new path is fully gated by `input_mode == 'expert_and_image_attn_adapter'`.
