...
def parse_arguments():
    ...
    parser.add_argument("--input_mode", type=str, default='image_only',
                       choices=['expert_and_image_attn', 'expert_and_image_attn_adapter', 'expert_and_image_concat', 'expert_and_image_add', 'expert_only', 'image_only', 'expert_cross_attn', 'expert_qformer'],
                       help="Input mode for the model. Main combinations: 'image_only' (with vanilla), 'expert_and_image_attn' (with expert), 'expert_and_image_attn_adapter' (with expert; same attn fusion plus a zero-init residual MLP adapter on the fused tokens), 'expert_cross_attn' (with expert; per-sample learnable cross-attention), or 'expert_qformer' (with expert; BLIP-2-style learnable-query two-stage fuser)")
    ...
    parser.add_argument("--training_parameters", nargs='+',
                       ...
                       choices=['llm', 'merger', 'expert_projector', 'expert', 'expert_fuser', 'post_fusion_adapter'],
...
if __name__ == "__main__":
    ...
    main_combinations = [
        ...
        ('expert_and_image_attn_adapter', 'expert'),
    ...
    if (config.input_mode, config.feature_mode) not in main_combinations:
        ...
        print("Main combinations are: ('image_only', 'vanilla'), ('expert_and_image_attn', 'expert'), ('expert_and_image_attn_adapter', 'expert'), ('expert_cross_attn', 'expert'), or ('expert_qformer', 'expert')")
