...
def _build_singleton_case(hh_id, id_id, hh_dict, stranger_pool,
                          embeddings, max_gallery):
    """Build one singleton distractor case for the given (hh_id, id_id) pair.

    Gallery contains up to ``max_gallery`` images of identity ``id_id``,
    selected by similarity to a stranger query drawn from a different
    household.
    """
...
def build_singleton_distractor(targets, stranger_pool, embeddings, max_gallery=5):
    """Real singleton distractor: gallery = images of the sole resident of a
    size-1 household."""
    if not targets:
        return None
    hh_id, hh_dict = random.choice(targets)
    id_id = list(hh_dict[hh_id].keys())[0]
    return _build_singleton_case(hh_id, id_id, hh_dict, stranger_pool,
                                 embeddings, max_gallery)


def build_synthetic_singleton_nondistr(hh_id, id_id, hh_dict, query_mac,
                                       camera, miner, max_gallery=5,
                                       tracker=None):
    """Synthetic non-distractor singleton case.

    Treats identity ``id_id`` as the lone "member" of household ``hh_id``:
    the gallery contains 1 image of ``id_id`` (different from the query)
    plus hard negatives padded up to a randomly chosen target_size. The
    miner excludes the entire household, so co-residents cannot leak into
    the gallery — this is what makes the case faithfully singleton.

    Mirrors ``build_nondistr_case`` but for a single identity drawn from a
    family household, used to augment the singleton-style non-distractor
    bucket beyond what real size-1 households provide.
    """
    id_macs = hh_dict[hh_id][id_id]

    q_imgs = id_macs.get(query_mac, {}).get('query', [])
    if not q_imgs:
        return None
    q_img = (tracker.pick(id_id, q_imgs) if tracker
             else random.choice(q_imgs))
    ...
    random.shuffle(gallery)
    label = next(i + 1 for i, (tag, _) in enumerate(gallery) if tag == id_id)
...
def add_query_status(cases):
    """Annotate each case with 'known' or 'unknown' based on label.

    'known'   = query identity is present in the gallery (label > 0,
                non-distractor case).
    'unknown' = query is a stranger and not in the gallery (label = -1,
                distractor case).
    """
    for case in cases:
        case['query_status'] = 'known' if case['label'] > 0 else 'unknown'
...
def build_training_cases(hh_dict, sampling_rates, miner, embeddings,
                         ...
                         add_synthetic_singletons=False):
    ...
    # ── Synthetic singleton augmentation ──────────────────────────────────
    # For each identity in a multi-person household, treat them as a
    # singleton and build BOTH:
    #   (a) one non-distractor case — gallery = 1 image of the identity +
    #       hard negs (co-residents excluded), label = position of the
    #       identity image. Mirrors real size-1 non-distractor protocol.
    #   (b) one distractor case — gallery = up to max_gallery images of
    #       the identity, query is a stranger from a different household,
    #       label = -1. Mirrors real size-1 distractor protocol.
    # Real size-1 households are skipped here — they already get sampled
    # in the regular bucket above. Bounding to one case per identity per
    # type keeps the data growth modest while contributing roughly
    # uniformly across gallery sizes 1..max_gallery.
    if add_synthetic_singletons:
        synthetic_candidates = [
            (hh_id, id_id)
            for hh_id in train_ids
            for id_id in hh_dict[hh_id]
            if len(hh_dict[hh_id]) > 1
        ]
        random.shuffle(synthetic_candidates)
        print(f"  Building synthetic singletons (1 non-distractor + 1 "
              f"distractor per family-household identity, "
              f"{len(synthetic_candidates)} candidates)...")
        nd_built, d_built = 0, 0
        for hh_id, id_id in synthetic_candidates:
            # Non-distractor: retry across camera/MAC choices to avoid
            # spurious failures (e.g. only one image in the chosen split).
            valid_macs = [m for m, s in hh_dict[hh_id][id_id].items()
                          if s.get('query') or s.get('gallery')]
            if valid_macs:
                for _ in range(20):
                    camera = random.choice(['same', 'cross'])
                    qmac = random.choice(valid_macs)
                    nd_case = build_synthetic_singleton_nondistr(
                        hh_id, id_id, hh_dict, qmac, camera,
                        miner=miner, max_gallery=max_gallery, tracker=tracker)
                    if nd_case:
                        non_distr.append(nd_case)
                        stats['synthetic_singleton_nondistr'] += 1
                        nd_built += 1
                        break

            # Distractor.
            d_case = _build_singleton_case(
                hh_id, id_id, hh_dict, stranger_pool, embeddings, max_gallery)
            if d_case:
                distr.append(d_case)
                stats['synthetic_singleton_distractor'] += 1
                d_built += 1
        print(f"    synthetic non-distractors: "
              f"{nd_built}/{len(synthetic_candidates)}")
        print(f"    synthetic distractors:     "
              f"{d_built}/{len(synthetic_candidates)}")
...
def main():
    ...
    parser.add_argument('--add_synthetic_singletons', action='store_true',
                        help='Add one synthetic singleton distractor per '
                             'identity in a family (size>1) household: the '
                             'gallery contains only that identity\'s images, '
                             'the query is a stranger from a different '
                             'household. Augments the singleton bucket '
                             'beyond what real size-1 households provide.')
    ...
    cases, stats = build_training_cases(
        ...
        add_synthetic_singletons=args.add_synthetic_singletons,
    )

    print("\n  Adding similarity scores, household IDs, and query status...")
    add_similarity_scores(cases, miner)
    add_household_ids(cases, metadata)
    add_query_status(cases)
    ...
    status_counts = Counter(c['query_status'] for c in cases)
    print(f"\n  Query status (known = label>0, unknown = label=-1):")
    for st in ('known', 'unknown'):
        print(f"    {st}: {status_counts.get(st, 0)} cases")
