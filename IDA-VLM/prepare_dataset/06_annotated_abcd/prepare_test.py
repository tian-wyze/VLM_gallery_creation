...
def build_distractor_subset(hh_same, hh_cross, hh_type, miner, embeddings, max_gallery):
    ...
    # Sort so the stranger pool is iteration-order-stable across runs;
    # bare set() iteration depends on Python's hash randomization and
    # makes random.choice non-reproducible even with random.seed set.
    all_household_ids = sorted(set(hh_same.keys()) | set(hh_cross.keys()))
...
def main():
    ...
    parser.add_argument(
        '--same_clothes',
        default='household_info_v2_same_clothes.json',
    )
    parser.add_argument(
        '--cross_clothes',
        default='household_info_v2_cross_clothes.json',
    )
