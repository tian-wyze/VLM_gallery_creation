# 06 — Annotated Identity Corpus + Lettered Options
...
```bash
cd IDA-VLM/prepare_dataset/06_annotated_abcd
...
Consumes `annotated_household_info_filtered.json` and writes `train_data.json` in the same case-level schema as [04_varying_gallery_length_distractors/](../04_varying_gallery_length_distractors/) (`{query, gallery, label, similarity, household_id, identity_id, query_household_id, query_identity_id, query_status}`). Reuses 04's `HardNegativeMiner`, `build_nondistr_case`, and distractor builders verbatim via direct import — only the input adapter, sampling rates, the dropping of the same/cross-clothes axis, the optional synthetic-singleton augmentation, and the `query_status` annotation differ.
...
# Plain training set (no synthesis)
python prepare_train.py

# With synthetic-singleton augmentation (recommended)
python prepare_train.py --add_synthetic_singletons --output train_data_with_synthetic_singletons.json
```

Defaults: `--max_gallery 6`, `--distractor_fraction 0.4`, `--seed 42`. DINOv2 ViT-L/14 embeddings are cached to `cached_embeddings/dinov2_embeddings.pt` (~2.1 GB) on first run; subsequent runs reuse the cache.

### `--add_synthetic_singletons`

For every identity in a multi-person household, the script adds **two extra cases** that treat that identity in isolation — as if they were the lone resident of a synthetic size-1 household:

- **Synthetic non-distractor**: gallery = 1 image of the identity + hard negatives (co-residents excluded). Mirrors the real size-1 non-distractor protocol from `build_nondistr_case`.
- **Synthetic distractor**: gallery = up to `max_gallery` images of the identity, query is a stranger from a different household, label = -1. Mirrors `build_singleton_distractor`.

Rationale: the model decides solely by comparing query and gallery appearances, so isolating one identity from a family household is a valid synthetic case (no co-occurrence cue is exposed in the gallery). The augmentation roughly **doubles** the singleton-style coverage without requiring more annotated data, and is target-distribution-friendly: each family identity contributes exactly two extra cases, distributed roughly uniformly across gallery sizes 1..max_gallery.

### `query_status`

Every saved case carries `query_status: "known" | "unknown"`:

- `"known"` — `label > 0`; the query identity is present in the gallery (non-distractor).
- `"unknown"` — `label = -1`; the query is a stranger (distractor).

Used downstream for stratified analysis and as a human-readable mirror of the integer `label`.
...
Run on **2026-04-27** with `--distractor_fraction 0.2` (no synthesis):
...
Re-run on **2026-05-01** with `--add_synthetic_singletons --distractor_fraction 0.4` (recommended):

| category | cases |
|---|---:|
| same-cam non-distractor (real)        | 21,281 |
| cross-cam non-distractor (real)       | 14,619 |
| distractor (real, 0.4 × non-distr)    | 14,360 |
| synthetic singleton non-distractor    | 15,454 |
| synthetic singleton distractor        | 15,454 |
| **Total**                             | **81,168** |
| of which `query_status == "known"`    | 51,354 |
| of which `query_status == "unknown"`  | 29,814 |

Gallery-length distribution (post hard-negative padding, `max_gallery = 6`):

| gallery size | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| cases (no synthesis) | 1,378 | 4,110 | 6,196 | 8,126 | 9,613 | 11,017 | 960 | 828 | 852 |
| cases (with `--add_synthetic_singletons`) | 6,724 | 9,656 | 11,732 | 13,970 | 16,047 | 18,005 | 1,758 | 1,626 | 1,650 |

Synthetic singletons can land in any size 1..6 (uniform `target_size`), so they specifically lift the under-represented small sizes — the worst-case 1-vs-6 ratio drops from 8× to ~2.7×. Sizes 7–9 only grow because the real-distractor budget grew with `distractor_fraction=0.4`.
...
train_data.json                                 # plain run output (legacy schema)
train_data_with_synthetic_singletons.json       # --add_synthetic_singletons output
cached_embeddings/
  dinov2_embeddings.pt                          # ~2.1 GB, 524,811 ViT-L/14 embeddings
```

The training pipeline does **not** consume these JSONs directly — it consumes the JSONL produced by `prepare_jsonl.py` (next section). Keep the JSON if you want the rich case-level fields for inspection; otherwise delete it after conversion.

---

# Letter-format conversion: `prepare_jsonl.py`

Converts a `prepare_train.py` or `prepare_test.py` output into a JSONL file where every case carries a randomized "stranger" option slot expressed as a letter (A, B, C, ...). The training pipeline uses the JSONL exclusively; the JSON files are intermediate.

## Why letter options + a randomized stranger slot

The model is trained to pick **one of N+1 lettered options** rather than emit an integer index. For a case with N gallery images, the prompt presents `N+1` options labelled A..(A+N): N of them are the gallery images, one is a text-only "(stranger / not in the gallery)" placeholder. Which letter the stranger placeholder occupies is randomized per case so the model cannot learn a fixed-position shortcut (e.g. "always last letter = stranger").

This subsumes the legacy `-1` sentinel: a distractor case is now just one where the correct answer letter happens to be the stranger slot.
...
```json
{
  ...
  "query_household_id": "...",
  "query_identity_id": "...",
  "query_status": "known" | "unknown",

  // added by prepare_jsonl.py
  "stranger_letter_pos": 1,    // 0..len(gallery), insertion index for the stranger slot
  "answer_letter": "C"         // letter the model must output
}
```

The training-side data loader derives the option layout by inserting a "stranger" placeholder at index `stranger_letter_pos` into the gallery sequence; the i-th option then gets letter `chr(ord('A') + i)`. `answer_letter` is the letter of either the matching gallery slot (for known cases) or the stranger slot (for unknown cases).

## Usage

Auto-detects two input shapes: flat list (`train_data*.json` from `prepare_train.py`) and dict-with-`eval_cases` (benchmark JSON from `prepare_test.py`). Default `--seed 42` for reproducibility.

The script prints a summary of the answer-letter and stranger-slot distributions so you can sanity-check that no letter dominates and the stranger slot is well-randomized.

### 1. Convert the training set

```bash
cd /home/tian.liu/IDA-VLM/prepare_dataset/06_annotated_abcd

python prepare_jsonl.py \
  --input  train_data_with_synthetic_singletons.json \
  --output train_data.jsonl
```

Expected output (with `--add_synthetic_singletons` upstream): 81,168 records, answer-letter distribution A=18,223 → J=162.

### 2. Convert all realistic benchmarks (no hard-neg padding)
...
Produces 10 `benchmarks/<scenario>.jsonl` files (sizes range from 150 to 462 records, matching `n_cases` in the source JSONs).

### 3. Convert all hard-negative benchmarks (stress test)

```bash
cd /home/tian.liu/IDA-VLM/prepare_dataset/06_annotated_abcd

for f in benchmarks_hardnegatives/*.json; do
  out="${f%.json}.jsonl"
  python prepare_jsonl.py --input "$f" --output "$out"
done
```

Produces 10 `benchmarks_hardnegatives/<scenario>.jsonl` files (same case counts as `benchmarks/`, but with galleries padded up to `--max_gallery 6` via DINOv2-mined hard negatives — see the `prepare_test.py` section below).

### 4. Single-file conversion (debugging)

If you just want to spot-check one file end-to-end:

```bash
cd /home/tian.liu/IDA-VLM/prepare_dataset/06_annotated_abcd

python prepare_jsonl.py \
  --input  benchmarks/cropped_sameclothes_family_samecamera.json \
  --output benchmarks/cropped_sameclothes_family_samecamera.jsonl
```

### Files produced

```
prepare_jsonl.py                          # this script
train_data.jsonl                          # ~80 MB, 81,168 cases (with --add_synthetic_singletons)
benchmarks/
  *.jsonl                                 # 10 realistic benchmarks (one per scenario)
benchmarks_hardnegatives/
  *.jsonl                                 # 10 stress-test benchmarks (gallery padded with hard negs)
```
...
```bash
TEST_FOLDER="../prepare_dataset/06_annotated_abcd/benchmarks"
```
...
05_annotated_train/                     ─→ earlier iteration of the annotated training set
06_annotated_abcd/  ◀── you are here
```

This folder consumes a **manually-curated** identity corpus (richer + cleaner than the auto-clustered datasets in 01–04) and produces both the training data and the test benchmarks for the **lettered-options** training format (`A:`, `B:`, ... with a randomized stranger slot). Pipeline:

1. `get_annotated_info.py` → `annotated_household_info_filtered.json` (parsed + household-disjoint).
2. `prepare_train.py [--add_synthetic_singletons]` → `train_data*.json` (case-level schema, integer labels).
3. `prepare_test.py` → `benchmarks/<scenario>.json` × 10 (case-level schema, integer labels).
4. `prepare_jsonl.py` → `train_data.jsonl` and `benchmarks/<scenario>.jsonl` (adds randomized stranger slot + `answer_letter`).

The JSONLs from step 4 are what the training pipeline actually consumes.
