        ...
        if pixel_values is not None and n_image_tokens > 0:
            ...
            if expert_inputs is not None:
                ...
                if expert_feature is not None:
                    ...
                    if expert_inputs['input_mode'] == 'expert_and_image_add':
                    ...
                    elif expert_inputs['input_mode'] in ('expert_and_image_attn', 'expert_and_image_attn_adapter'):
                        ...
                        # Optional post-fusion refinement: zero-init residual MLP
                        # adapter applied to the fused tokens before they reach the
                        # LLM. Identity at init, so adding the mode is a no-op
                        # until training pushes the adapter weights off zero.
                        if expert_inputs['input_mode'] == 'expert_and_image_attn_adapter':
                            image_embeds = self.visual.post_fusion_adapter(image_embeds)
