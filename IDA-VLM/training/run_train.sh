...
PREFIX="postadapter-annotated-distractor-sft-qwen7b-WYZEv04_15_token"
...
INPUT_MODE="expert_and_image_attn_adapter"  # attn fusion + zero-init residual MLP adapter on the fused tokens
...
# - `post_fusion_adapter`: the zero-init residual MLP adapter applied to the
#   fused image tokens. Must be present for expert_and_image_attn_adapter
#   or the adapter stays at its zero init (identity) and the new mode is a
#   no-op. Harmless to include for other modes (the module is None then,
#   so no parameter names match).
# TRAINING_PARAMETERS="merger expert_projector expert_fuser"
# For input_mode=expert_and_image_attn_adapter, use:
