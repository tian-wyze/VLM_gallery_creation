...
MODELS=(
    ...
    # post-adapter
    "/home/tian.liu/IDA-VLM/training/runs/20260522_012735_postadapter-annotated-distractor-sft-qwen7b-WYZEv04_15_token_Qwen2.5-VL-7B-Instruct_person_expert_wyzev0415token_expert_and_image_attn_adapter_lr_0.0002_bs_4_captions_False/ckpts/connector_best_step_28000.pt"
...
RESULTS_FILE="results/results_postadapter_annotated_abcd_Qwen2.5-VL-7B_FTwyze.csv"
...
for MODEL in "${MODELS[@]}"; do
    ...
    # Input_modes have 2, 3, 4, or 5 tokens; try longest first so e.g.
    # "expert_and_image_attn_adapter" (5) wins over "expert_and_image_attn" (4),
    # and "expert_and_image_attn" (4) wins over any 3-token suffix ending in "attn".
    INPUT_START=""
    for n in 5 4 3 2; do
        START=$((LR_IDX-n))
        CAND=$(IFS='_'; echo "${PARTS[*]:$START:$n}")
        case "$CAND" in
            image_only|expert_only|expert_qformer|expert_and_image_attn|expert_and_image_attn_adapter|expert_and_image_concat|expert_and_image_add|expert_cross_attn)
                INPUT_START=$START
                break
                ;;
        esac
